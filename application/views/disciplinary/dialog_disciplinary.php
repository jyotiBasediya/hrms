...
if(isset($_GET['jd']) && isset($_GET['disciplinary_id']) && $_GET['data']=='disciplinary'){
?>

<div class="modal-header">
  <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
  <h4 class="modal-title" id="edit-modal-data"><?php echo $this->lang->line('hrms_edit_disciplinary');?></h4>
</div>
<form class="m-b-1" action="<?php echo site_url("disciplinary/update").'/'.$disciplinary_id; ?>" method="post" name="edit_disciplinary" id="edit_disciplinary">
  <input type="hidden" name="_method" value="EDIT">
  <input type="hidden" name="_token" value="<?php echo $disciplinary_id;?>">
  ...
  <div class="modal-body">
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" name="title" placeholder="<?php echo $this->lang->line('hrms_title');?>" value="<?php echo $title;?>">
    </div>
    <div class="form-group">
      <label for="message"><?php echo $this->lang->line('hrms_description');?></label>
      <textarea class="form-control" placeholder="<?php echo $this->lang->line('hrms_description');?>" name="description" id="description2"><?php echo $description;?></textarea>
    </div>
  </div>
...
<script type="text/javascript">
 $(document).ready(function(){
        ...
        "bDestroy": true,
		"ajax": {
            url : "<?php echo site_url("disciplinary/disciplinary_list") ?>",
            type : 'GET'
        },
		...
		/* Edit data */
		$("#edit_disciplinary").submit(function(e){
		e.preventDefault();
			...
			$.ajax({
				type: "POST",
				url: e.target.action,
				data: obj.serialize()+"&is_ajax=1&edit_type=disciplinary&form="+action,
				cache: false,
...
<?php } else if(isset($_GET['jd']) && isset($_GET['disciplinary_id']) && $_GET['data']=='view_disciplinary'){ ?>
<div class="modal-header">
  <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
  <h4 class="modal-title" id="edit-modal-data"><?php echo $this->lang->line('hrms_view_disciplinary');?></h4>
</div>
<form class="m-b-1">
  <div class="modal-body">
    <div class="form-group">
