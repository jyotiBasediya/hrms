           ...
           <div class="content-wrapper">   
                <div class="page-header">
                   <div class="d-flex justify-content-between w-100">
                    ...
                    <a class="btn btn-primary icon-btn" href="<?php echo site_url('CmsController/add_news');?>"><i class="fa fa-plus"></i>Add News	</a>
                     <?php } ?>
                      ...
                      <thead>
                        <tr>
                          <th>S.No</th>
                                    ...
                                    <th>Description</th>
                                    <th>Status</th>
                                     <?php if(in_array('54',$role_resources_ids) || in_array('56',$role_resources_ids) || in_array('57',$role_resources_ids)) { ?>
                                    <th>Action</th>
                                     <?php } ?>
                        </tr>
                      </thead>
                        ...
                        <!-- <td>hhdigg</td> -->
                          ...
                          <td>
                             <?php if(in_array('54',$role_resources_ids)) { ?>
				                        <a  title="View" href="<?php echo base_url().'CmsController/view_news/'.$value['id'];?>" class="btn btn-primary edit_add"><i class="fa fa-eye"></i></a>
                                 <?php } if(in_array('56',$role_resources_ids)) { ?>
                                        <a  title="Edit" href="<?php echo base_url().'CmsController/edit_news/'.$value['id'];?>" class="btn btn-primary edit_add"><i class="fa fa-pencil"></i></a>
                                   <?php } if(in_array('57',$role_resources_ids)) { ?>      
                                        <a  title="Delete" href="<?php echo base_url().'CmsController/delete_news/'.$value['id'];?>" onclick="return deleteNews()" class="btn btn-danger">
...
<!-- 
 ...
 <script>	
 function deleteNews(){
        var result = confirm("Are sure delete this News ?");
        if(result == true){
            return true;
        } 
        else{
      ...
      function change_status(id,value)
    {
        var news_id = id;
            // alert(user_id);
        if(confirm("Are you sure want "+value+" this News")){
            $.ajax({
                url: '<?php echo site_url("CmsController/news_status"); ?>',
                type: "POST",
                data: {
                    "news_id" : news_id
                },
                success: function(response) { 
                    ...
                    if(data.status == 1)
                    {
                        $('#change_status_'+data.news_id).removeClass("btn-info").addClass('btn-success').text('Active')
                    }
                    else 
                    {
                        $('#change_status_'+data.news_id).removeClass("btn-success").addClass('btn-info').text('Inactive')
                    }
                    location.reload();
                }
            });
