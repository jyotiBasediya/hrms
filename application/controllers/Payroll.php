...
class Payroll extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function pdf_create() {
		...
		$user = $this->Hrms_model->read_user_info($payment[0]->employee_id);
		$_des_name = $this->Designation_model->read_designation_information($user[0]->designation_id);
		$department = $this->Department_model->read_department_information($user[0]->department_id);
		$location = $this->Hrms_model->read_location_info($department[0]->location_id);
		// company info
		$company = $this->Hrms_model->read_company_info($location[0]->company_id);
		...
		$country = $this->Hrms_model->read_country_info($company[0]->country);
		...
		if($payment[0]->house_rent_allowance!='' || $payment[0]->house_rent_allowance!=0){
			$hra = $this->Hrms_model->currency_sign($payment[0]->house_rent_allowance);
		} else { $hra = '0';}
		if($payment[0]->medical_allowance!='' || $payment[0]->medical_allowance!=0){
			$ma = $this->Hrms_model->currency_sign($payment[0]->medical_allowance);
		} else { $ma = '0';}
		if($payment[0]->travelling_allowance!='' || $payment[0]->travelling_allowance!=0){
			$ta = $this->Hrms_model->currency_sign($payment[0]->travelling_allowance);
		} else { $ta = '0';}
		if($payment[0]->dearness_allowance!='' || $payment[0]->dearness_allowance!=0){
			$da = $this->Hrms_model->currency_sign($payment[0]->dearness_allowance);
		} else { $da = '0';}
		
		// Deductions
		if($payment[0]->provident_fund!='' || $payment[0]->provident_fund!=0){
			$pf = $this->Hrms_model->currency_sign($payment[0]->provident_fund);
		} else { $pf = '0';}
		if($payment[0]->tax_deduction!='' || $payment[0]->tax_deduction!=0){
			$td = $this->Hrms_model->currency_sign($payment[0]->tax_deduction);
		} else { $td = '0';}
		if($payment[0]->security_deposit!='' || $payment[0]->security_deposit!=0){
			$sd = $this->Hrms_model->currency_sign($payment[0]->security_deposit);
		} else { $sd = '0';}
		...
		<table cellpadding="5" cellspacing="0" border="1">
			<tr style="background-color:#c4e5fd;">
			 ...
			 <tr>
				...
				<td colspan="2" align="right">'.$this->Hrms_model->currency_sign($payment[0]->basic_salary).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Gross Salary</td>
				<td align="right">'.$this->Hrms_model->currency_sign($payment[0]->gross_salary).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Total Allowance</td>
				<td align="right">'.$this->Hrms_model->currency_sign($payment[0]->total_allowances).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Total Deduction</td>
				<td align="right">'.$this->Hrms_model->currency_sign($payment[0]->total_deductions).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Net Salary</td>
				<td align="right">'.$this->Hrms_model->currency_sign($payment[0]->net_salary).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Paid Amount</td>
		...
		<table cellpadding="5" cellspacing="0" border="1">
			...
			<tr>
				...
				<td colspan="2" align="right">'.$this->Hrms_model->currency_sign($payment[0]->hourly_rate).'</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Gross Salary</td>
				<td align="right">'.$this->Hrms_model->currency_sign($payment[0]->payment_amount).'</td>
			</tr>
			<tr>
				<td colspan="2">&nbsp;</td>
				<td>Net Salary</td>
    ...
    public function template_list()
     {
		...
		$template = $this->Payroll_model->get_templatesByCmp();
		...
		$data = array();
          ...
          foreach($template as $r) {
			  ...
			  // get basic salary
			  $sbs = $this->Hrms_model->currency_sign($r->basic_salary);
			  // get net salary
			  $sns = $this->Hrms_model->currency_sign($r->net_salary);
			  // get date > created at > and format
			  $cdate = $this->Hrms_model->set_date_format($r->created_at);
			  // total allowance
				if($r->total_allowance == 0 || $r->total_allowance=='') {
					$allowance = '-';
				} else{
					$allowance = $this->Hrms_model->currency_sign($r->total_allowance);
				}
          ...
          $output = array(
               "draw" => $draw,
                 "recordsTotal" => count($template),
                 "recordsFiltered" => count($template),
                 "data" => $data
            );
	 ...
	 public function payment_history_list()
     {
		...
		$history = $this->Payroll_model->all_payment_historyByCmp();
		...
		$data = array();
          ...
          foreach($history as $r) {
			  ...
			  $p_amount = $this->Hrms_model->currency_sign($r->payment_amount);
	
			  // get date > created at > and format
			  $created_at = $this->Hrms_model->set_date_format($r->created_at);
			   // get hourly rate
			  // payslip
	 ...
	 public function hourly_list()
     {
		...
		$hourly_wages = $this->Payroll_model->get_hourly_wagesByCmp();
		
		$data = array();

          foreach($hourly_wages as $r) {
			  ...
			  $cdate = $this->Hrms_model->set_date_format($r->created_at);
			   // get hourly rate
			  $hourly_rate = $this->Hrms_model->currency_sign($r->hourly_rate);
          ...
          $output = array(
               "draw" => $draw,
                 "recordsTotal" => count($hourly_wages),
                 "recordsFiltered" => count($hourly_wages),
                 "data" => $data
            );
          echo json_encode($output);
          exit();
	 ...
	 public function payslip_list()
     {
		...
		if($this->input->get("employee_id")){
			...
			$employee_id = $this->input->get("employee_id");
			$p_date = $this->input->get("month_year");
			$payslip = $this->Employees_model->get_employee_withCompany($session['companyId']);
		}
		...
		$data = array();
          ...
          foreach($payslip as $r) {
			...
			if($r->monthly_grade_id =='' || $r->monthly_grade_id ==0) {
				...
				if(isset($grade_template[0]->salary_grades)){
          ...
          $output = array(
               "draw" => $draw,
                 "recordsTotal" => count($payslip),
                 "recordsFiltered" => count($payslip),
                 "data" => $data
            );
          echo json_encode($output);
          exit();
	 ...
	 public function salary_list()
     {
		...
		if($this->input->get("employee_id")) {
			...
			$salary = $this->Employees_model->get_employee_withCompany($session['companyId']);
		}
		...
		$selected = '';
		foreach($this->Payroll_model->all_hourly_templatesByCmp() as $hourly_template){
		...
		if(($r->monthly_grade_id ==0 || $r->hourly_grade_id=='') && ($r->hourly_grade_id ==0 || $r->hourly_grade_id=='')) {
          ...
          $output = array(
               "draw" => $draw,
                 "recordsTotal" => count($salary),
                 "recordsFiltered" => count($salary),
                 "data" => $data
            );
          echo json_encode($output);
          exit();
	...
	public function pay_monthly()
	{
        ...
        // get addd by > template
		$user = $this->Hrms_model->read_user_info($id);
	...
	$session = $this->session->userdata('username');
		...
		} else if(!is_numeric($this->input->post('basic_salary'))) {
			$Return['error'] = "The basic salary field isn't containing valid amount.";
		}
	...
	public function add_pay_monthly() {
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				//get email template
				$template = $this->Hrms_model->read_email_template(1);
				//get employee info
				$user_info = $this->Hrms_model->read_user_info($this->input->post('emp_id'));
	...
	public function benefits()
	{
		...
		$data['title'] = $this->Hrms_model->site_title();
		$session = $this->session->userdata('username');
		$data['breadcrumbs'] = 'Benefits';
		$data['path_url'] = 'benefits';
		$role_resources_ids = $this->Hrms_model->user_role_resource();
		if(in_array('42',$role_resources_ids)) {
			if(!empty($session)){
				$data['all_employees'] = $this->Hrms_model->all_employees_byCmpId($session['companyId']);
				$data['benefit_types'] = $this->Payroll_model->all_benefit_types();
				$data['subview'] = $this->load->view("payroll/benefits", $data, TRUE);
				$this->load->view('layout_main', $data); //page load
			} else {
				redirect('');
			}
	...
	public function benefits_list()
	{
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("payroll/benefits", $data);
		} else {
			redirect('');
		}
		// Datatables Variables
		...
		$benefit = $this->Payroll_model->get_benefitsByCmp();
		...
		$data = array();
          ...
          foreach($benefit as $r) {
			  ...
			  $user = $this->Hrms_model->read_user_info($r->added_by);
			  // user full name
			  $added_by = $user[0]->first_name.' '.$user[0]->last_name;
			  ...
			  // get benefit amount
			  $amount = $this->Hrms_model->currency_sign($r->amount);

			  // get benefit type
			  $benefit_type = $this->Payroll_model->read_benefit_type($r->benefit_type_id);

			  if(!empty($benefit_type)){
			  	$benefit_type_name = $benefit_type[0]->benefit_type;
			  } else {
			  	$benefit_type_name = '';
			  }
			  ...
			  // get date > start date > and format
			  $start_date = $this->Hrms_model->set_date_format($r->start_date);

			  // get date > end date > and format
			  $end_date = $this->Hrms_model->set_date_format($r->end_date);

               $data[] = array(
			   		'<span data-toggle="tooltip" data-placement="top" title="Edit"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-benefit_id="'. $r->benefit_id . '"><i class="fa fa-pencil-square-o"></i></button></span><span data-toggle="tooltip" data-placement="top" title="" data-original-title="View"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-benefit_id="'. $r->benefit_id . '"><i class="fa fa-eye"></i></button></span><span data-toggle="tooltip" data-placement="top" title="Delete"><button type="button" class="btn btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->benefit_id . '"><i class="fa fa-trash-o"></i></button></span>',
			   		$employee_name,
			   		$benefit_type_name,
                    $start_date,
                    $end_date,
          ...
          $output = array(
               "draw" => $draw,
                 "recordsTotal" => count($benefit),
                 "recordsFiltered" => count($benefit),
                 "data" => $data
            );
          echo json_encode($output);
          exit();
	...
	{
		if($this->input->post('add_type')=='benefit') {
			/* Define return | here result is used to return user data and error for error message */
			$Return = array('result'=>'', 'error'=>'');
			...
			if($this->input->post('employee_id')==='') {
			...
			} else if($this->input->post('benefit_type_id')==='') {
				$Return['error'] = "The benefit type field is required.";
				...
				$Return['error'] = "The end date field is must be greater then start date.";
			} else if($this->input->post('amount')==='') {
				$Return['error'] = "The amount field is required.";
			} else if(!is_numeric($this->input->post('amount'))) {
				$Return['error'] = "The amount field isn't containing valid amount.";
			}
			...
			$result = $this->Payroll_model->add_benefit($data);
			if ($result == TRUE) {
				$Return['result'] = 'Employee benefit added.';
			} else {
				$Return['error'] = 'Bug. Something went wrong, please try again.';
			}
			$this->output($Return);
	...
	// get payroll template info by id
	public function benefit_read()
	{
		$session = $this->session->userdata('username');
		$data['title'] = $this->Hrms_model->site_title();
		$id = $this->input->get('benefit_id');
       // $data['all_countries'] = $this->Hrms_model->get_countries();
		$result = $this->Payroll_model->read_benefit_information($id);
		$data = array(
				'benefit_id' => $result[0]->benefit_id,
				'benefit_type_id' => $result[0]->benefit_type_id,
		...
		if(!empty($session)){ 
			$this->load->view('payroll/dialog_benefits', $data);
		} else {
			redirect('');
		}
	}
	...
	public function benefit_view()
	{
       ...
       // $data['all_countries'] = $this->Hrms_model->get_countries();
		$result = $this->Payroll_model->read_benefit_information($id);
		$benefit_type = $this->Payroll_model->read_benefit_type($result[0]->benefit_type_id);
		$user = $this->Employees_model->read_employee_information($result[0]->employee_id);
		...
		$data = array(
				'benefit_id' => $result[0]->benefit_id,
				'benefit_type' => $benefit_type[0]->benefit_type,
				'employee_name' => $user[0]->first_name.' '.$user[0]->last_name,
				'start_date' => $result[0]->start_date,
				'end_date' => $result[0]->end_date,
				'amount' => $result[0]->amount,
	...
	public function update_benefit() {
		...
		if($this->input->post('edit_type')=='benefit') {
			...
			$result = $this->Payroll_model->update_benefit_record($data,$id);
			if ($result == TRUE) {
				$Return['result'] = 'Employee benefit added.';
			} else {
				$Return['error'] = 'Bug. Something went wrong, please try again.';
	...
	public function delete_benefit()
	{
		$Return = array('result'=>'', 'error'=>'');
		$id = $this->uri->segment(3);
		$result = $this->Payroll_model->delete_benefit_record($id);
		if(isset($id)) {
			$Return['result'] = 'Benefit deleted.';
		} else {
			$Return['error'] = 'Bug. Something went wrong, please try again.';
		}
		$this->output($Return);
