...
class Tickets extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['all_employees'] = $this->Hrms_model->all_employees_byCmpId($cmpID);
    ...
    public function ticket_list()
     {
		...
		$ticket = $this->Tickets_model->get_tickets_byCmp();
		...
		if($r->ticket_priority==1): $priority = 'Low'; elseif($r->ticket_priority==2): $priority = 'Medium'; elseif($r->ticket_priority==3): $priority = 'High'; elseif($r->ticket_priority==4): $priority = 'Critical';  endif;
		 ...
		 $edate = $this->Hrms_model->set_date_format($_date[0]);
	...
	public function add_ticket() {
		...
		$empData = $this->Hrms_model->select_single_row('select * from hrms_employees where user_id = "'.$this->input->post('employee_id').'"');
		if (!empty($empData)) 
		{
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				//get email template
				$template = $this->Hrms_model->read_email_template(15);
				//get employee info
	...
	public function set_comment() {
		...
		$hrms_comment = $this->input->post('xin_comment');
		...
		$data = array(
		'ticket_comments' => $qt_hrms_comment,
	...
	public function details()
     {
		...
		$data = array(
				...
				'all_employees' => $this->Hrms_model->all_employees_byCmpId($cmpID),
				);
			/*	print_r($data);
				die();*/
